...
# Set random 0 to the image
X_train_noisy = X_train * np.random.binomial(1,0.5*np.ones(784)).reshape(28,28) 
X_test_noisy = X_test * np.random.binomial(1,0.5*np.ones(784)).reshape(28,28) 
...
for i in range(n_images):
    ...
    plt.title('MNIST Example Noisy Image ' + str(i + 1))
    plt.show()
...
#########
history = model.fit(X_train_noisy, X_train, batch_size, epochs,validation_split=0.2,validation_data=(X_test_noisy,X_test),shuffle=True)
